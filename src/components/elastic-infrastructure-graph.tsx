...
export function ElasticInfrastructureGraph() {
	...
	useEffect(() => {
		...
		const generateDemandPoint = (x: number) => {
			...
			return Math.max(0.1, Math.min(0.9, base + variation1 + variation2 + variation3 + spike))
		}
		...
		const animate = () => {
			...
			const textColor = isInDarkMode ? "rgba(168, 205, 255, 0.6)" : "rgba(100, 116, 139, 0.8)" // muted-foreground
			const gridColor = isInDarkMode ? "rgba(168, 205, 255, 0.1)" : "rgba(100, 116, 139, 0.1)"
			const axisColor = isInDarkMode ? "rgba(168, 205, 255, 0.3)" : "rgba(100, 116, 139, 0.3)"
