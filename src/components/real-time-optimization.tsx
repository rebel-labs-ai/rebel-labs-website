	...
	useEffect(() => {
		...
		const animate = () => {
			...
			const textColor = isInDarkMode
				? "rgba(168, 205, 255, 0.8)"
				: "rgba(100, 116, 139, 0.9)"
			const nodeColor = isInDarkMode ? "#A1BCD1" : "#8AACCC"
			const pathColor = isInDarkMode
				? "rgba(168, 205, 255, 0.3)"
				: "rgba(100, 116, 139, 0.3)"
			const glowColor = isInDarkMode
				? "rgba(168, 205, 255, 0.6)"
				: "rgba(161, 188, 209, 0.6)"
			...
			if (currentPhase === "action" && actionProgress > 0) {
				...
				const pulseY =
					workforceY + 20 + (actionY - 20 - workforceY - 20) * actionProgress
			...
			if (currentPhase === "outcome" && outcomeProgress > 0) {
				const pulseX =
					actionX + 20 + (outcomeX - 20 - actionX - 20) * outcomeProgress
			...
			if (currentPhase === "feedback" && feedbackProgress > 0) {
				const pulseX =
					outcomeX + (workforceX + 20 - outcomeX) * feedbackProgress
				const pulseY =
					outcomeY - 20 + (workforceY + 20 - outcomeY + 20) * feedbackProgress
			...
			ctx.stroke()
			ctx.fillStyle = isInDarkMode
				? "rgba(161, 188, 209, 0.1)"
				: "rgba(161, 188, 209, 0.2)"
			ctx.fill()
			...
			if (currentPhase === "action") {
				...
				if (feedbackProgress >= 1) {
					...
					if (cycleCount % 3 === 0) {
						...
						setInsight(
							"Insight: Pattern 'B' is 3% more effective.\nAdopting as new standard."
						)
